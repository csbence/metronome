 ...
 private:
  ...
  class Node {
   public:
    ...
    mutable std::size_t openQueueIndex;
    /** Index used by the priority queue */
    mutable std::size_t safetyQueueIndex;
  ...
  const Node* explore(const State& startState,
    ...
    initializeLss(startState);

    int costBucket = 10;
    int currentExpansionDuration = 0;
    ...
    while (!terminationChecker.reachedTermination() && openList.isNotEmpty()) {
      ...
      bool attemptSafetyProof = currentExpansionDuration >= costBucket;

      if (attemptSafetyProof) {
        currentExpansionDuration = 0;

        // Create derived termination checker
        int safetyExpansionCount = proveSafety(listTopNode, terminationChecker);

        if (listTopNode->safe) {
          costBucket = 10;
          safeNodes.push_back(listTopNode);
        } else {
          costBucket *= 2;
        }
      } else {
  ...
  void initializeLss(const State& startState) {
    ++iterationCounter;
    clearOpenList();
    ...
    if (startNode == nullptr) {
      startNode = nodePool.construct(Node{nullptr,
                                          ...
                                          true});
      startNode->safetyDistance = domain.safetyDistance(startState);
    } else {
      startNode->g = 0;
      startNode->action = this->Action();
  ...
  int proveSafety(Node* sourceNode, TerminationChecker& terminationChecker) {
    safetyQueue.clear();
    safetyQueue.push(sourceNode);
    
    // TODO remove open flag and use queue
    
    // Do while we don't run out of time or until the we find a safe node
  ...
  Node* createNode(Node* sourceNode, SuccessorBundle<Domain> successor) {
    ...
    newNode->safetyDistance = domain.safetyDistance(successor.state);
  ...
  struct NodeComparatorF {
    int operator()(const Node* lhs, const Node* rhs) const {
      if (lhs->g + lhs->h < rhs->g + rhs->h) return -1;
      if (lhs->g + lhs->h > rhs->g + rhs->h) return 1;
      if (lhs->g > rhs->g) return -1;
      if (lhs->g < rhs->g) return 1;
      return 0;
  ...
  struct NodeOpenQueueIndex {
    std::size_t& operator()(Node* node) const { return node->openQueueIndex; }
  };

  struct NodeSafetyQueueIndex {
    std::size_t& operator()(Node* node) const { return node->safetyQueueIndex; }
  };
