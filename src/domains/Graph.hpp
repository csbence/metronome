...
public:
    ...
    public:
        ...
        Action(std::size_t sourceStateId, std::size_t targetStateId, Cost actionCost)
                : sourceStateId{sourceStateId}, targetStateId{targetStateId}, actionCost{actionCost} {}
        ...
        static Action getIdentity() { throw MetronomeException("Graph does not have an identity action"); }

        std::string toString() const {
            return "s: " + std::to_string(sourceStateId) + " t: " + std::to_string(targetStateId) + " c: " +
                    std::to_string(actionCost);
        }

        bool operator==(const Action& action) const {
            return action.sourceStateId == sourceStateId && action.targetStateId == targetStateId &&
    ...
    Cost distance(const State& state) const { return nodes[state.getId()].distance; }

    Cost heuristic(const State& state) const { return nodes[state.getId()].heuristic; }
    ...
    std::vector<SuccessorBundle<Graph>> successors(const State& state) const {
        ...
        for (auto edge : sourceNode.edges) {
            successors.emplace_back(State{edge.target}, Action{sourceNodeId, edge.target, edge.cost}, edge.cost);
        }
...
private:
    ...
    struct Edge {
        Edge() : id{0} {}
        Edge(size_t id, size_t target, Cost cost) : id{id}, target{target}, cost{cost} {}
        size_t id;
        size_t target{0};
        Cost cost{0};
    };
    ...
    void parseNode(const std::vector<std::string>& tokens) {
        ...
        addNode((size_t)std::stoi(tokens[1]), std::stoi(tokens[2]), std::stoi(tokens[3]));
    }

    void parseAttributes(const std::vector<std::string>& tokens) {
        if (tokens[0] != "graph") {
            throw MetronomeException("Graph attributes can't start with: " + tokens[0]);
        }
