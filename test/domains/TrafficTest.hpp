...
metronome::Traffic testGrid =
    metronome::ConfigurationExecutor::extractDomain<metronome::Traffic>(
        metronome::Configuration(json), resourceDir);
...
const std::string resourceDir2 =
    "/home/aifs2/doylew/Public/metronome/resources";
metronome::GridWorld testGrid2 =
    metronome::ConfigurationExecutor::extractDomain<metronome::GridWorld>(
        metronome::Configuration(json2), resourceDir2);

TEST_CASE("Traffic basic creation test", "[Traffic]") {
  //    metronome::Traffic traffic = testGrid;
  //    std::cout << testGrid.getStartState().getX() << "," <<
  //    testGrid.getStartState().getY() << std::endl; std::cout <<
  //    testGrid.getGoalState().getX() << "," << testGrid.getGoalState().getY()
  //    << std::endl; std::cout << testGrid.isGoal(testGrid.getStartState()) <<
  //    std::endl; std::cout << testGrid.isGoal(testGrid.getGoalState()) <<
  //    std::endl;
  //
  //    testGrid.visualize(std::cout, testGrid.getStartState(),
  //    metronome::Traffic::Action(5));
  //
  //    std::vector<SuccessorBundle<metronome::Traffic>> test =
  //    testGrid.successors(testGrid.getStartState());
  //
  //    std::cout << "SUCCESSORS: \n" << std::endl;
  ...
  //        std::vector<SuccessorBundle<metronome::Traffic>> test1 =
  //        testGrid.successors(bundle.state);
  //
  ...
  //            std::vector<SuccessorBundle<metronome::Traffic>> test2 =
  //            testGrid.successors(bundle1.state);
  //
  //            for (SuccessorBundle<metronome::Traffic> bundle2 : test2) {
  //                testGrid.visualize(std::cout, bundle2.state,
  //                bundle2.action); std::cout << "SUCCESSORS OF SUCCESSORS
  //                SUCCESSORS of successors" << std::endl;
  //                std::vector<SuccessorBundle<metronome::Traffic>> test3 =
  //                testGrid.successors(bundle2.state);
  //
  //                for (SuccessorBundle<metronome::Traffic> bundle3 : test3) {
  //                    testGrid.visualize(std::cout, bundle3.state,
  //                    bundle3.action);
  //                }
  ...
  metronome::SZero<metronome::Traffic, metronome::ExpansionTerminationChecker>
      SZero(testGrid, config);

  metronome::LssLrtaStar<metronome::GridWorld,
                         metronome::TimeTerminationChecker>
      LssLrtaStar(testGrid2, config);

  metronome::AStar<metronome::Traffic> aStar(testGrid, config);

  std::vector<metronome::Traffic::Action> rete =
      aStar.plan(testGrid.getStartState());
  LOG(INFO) << "Astar Solution: " << std::endl;
  for (auto i : rete) {
    LOG(INFO) << i.toString() << std::endl;
  }

  std::vector<metronome::OnlinePlanner<
      metronome::Traffic,
      metronome::ExpansionTerminationChecker>::ActionBundle>
      ret = SZero.selectActions(testGrid.getStartState(), check);

  std::vector<
      metronome::OnlinePlanner<metronome::GridWorld,
                               metronome::TimeTerminationChecker>::ActionBundle>
      rett = LssLrtaStar.selectActions(testGrid2.getStartState(), checker);

  //    LOG(INFO) << "S: " << ret.size() << " SZero Solution: " << std::endl;
  //    for (auto i : ret) {
  //        LOG(INFO) << i.action.toString() << std::endl;
  //    }
  //    LOG(INFO) << "S: " << rett.size() << " LssLrtaStar Solution: " <<
  //    std::endl; for (auto i : rett) {
  //        LOG(INFO) << i.action.toString() << std::endl;
  //    }
}
...
TEST_CASE("Traffic simulation", "[Traffic]") {
  ...
  for (auto action : actions) {
    auto candidateState = testGrid.transition(
        currentState, metronome::Traffic::Action::toValue(action.c_str()));
    ...
    testGrid.visualize(
        std::cout,
        currentState,
        metronome::Traffic::Action{
            metronome::Traffic::Action::toValue(action.c_str())});
  }
}
}  // namespace
